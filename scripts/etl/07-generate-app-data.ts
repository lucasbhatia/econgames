...
function main() {
  ...
  const trackPresets: Record<string, Array<{
    ...
    consistency: number; lateAcceleration: number; avgFinish: number;
  }>> = {};

  for (const trackId of TRACK_IDS) {
    const trackRaces = gpsRaces.filter(r => r.track_id.trim() === trackId);
    // Aggregate per horse
    const horseMap = new Map<string, { name: string; races: number; totalSpeed: number; topSpeed: number; totalEff: number; totalEarly: number; totalLateAccel: number; curves: number[][]; finishes: number[] }>();
    for (const r of trackRaces) {
      const name = r.horse_name.trim();
      if (!name) continue;
      if (!horseMap.has(name)) horseMap.set(name, { name, races: 0, totalSpeed: 0, topSpeed: 0, totalEff: 0, totalEarly: 0, totalLateAccel: 0, curves: [], finishes: [] });
      ...
      if (r.official_position > 0) h.finishes.push(r.official_position);
    }
    ...
    trackPresets[trackId] = sorted.map(h => {
      ...
      const lateAccel = Math.round((h.totalLateAccel / h.races) * 100) / 100;
      const style = earlyPos <= 2.5 ? "Front Runner" : earlyPos <= 5.5 ? "Stalker" : "Closer";
      // Compute consistency from finish position variance (same formula as profileToSim)
      const avgFinish = h.finishes.length > 0 ? h.finishes.reduce((s, v) => s + v, 0) / h.finishes.length : 5;
      const finishVariance = h.finishes.length > 1
        ? h.finishes.reduce((s, v) => s + (v - avgFinish) ** 2, 0) / h.finishes.length
        : 4; // default moderate variance
      const finishStdev = Math.sqrt(finishVariance);
      const consistency = Math.min(0.9, Math.max(0.2, 0.2 + finishStdev * 0.175));
